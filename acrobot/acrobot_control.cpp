...
//ca::Function get_integrator(
//    const bool& use_cvodes, const ca::SXDict& dae, const ca::SX& X,
//    const ca::SX& U, const ca::SX& ode, const ca::SX& L, const double& T, const int& N) {
//  ca::Function F;
//  if (use_cvodes) {
...
//    const auto f = ca::Function("integrator", {X, U}, {ode, L},
//                                              {"x", "u"}, {"dx", "l"});
//
...
ca::DMDict multiple_shooting(const Acrobot& acrobot,
  ...
  const auto F = integrator("integrator", "cvodes", dae, {{"t0", 0}, {"tf", T / N}});

  // initial and final state
  const std::vector<double> Xi(nx, 0.0);
  std::vector<double> Xf(nx, 0.0);
  Xf[0] = M_PI;
  ...
  const std::vector<double> lbU(nu, -ca::inf);
  const std::vector<double> ubU(nu, ca::inf);
  const std::vector<double> U_guess(nu, 0);
...
int main() {
  ...
  const int N = 100; // # shooting nodes == # control ticks
